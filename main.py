...
MAIN_GAME_SOUND = pygame.mixer.Sound(os.path.join("Assets", "Space-Jazz.mp3"))
BULLET_HIT_SOUND = pygame.mixer.Sound(os.path.join("Assets", "Grenade+1.mp3"))
BULLET_FIRE_SOUND = pygame.mixer.Sound(os.path.join("Assets", "Gun+Silencer.mp3"))
WIN_SOUND = pygame.mixer.Sound(os.path.join("Assets", "Win.wav"))
WIN_CHEER_SOUND = pygame.mixer.Sound(os.path.join("Assets", "Win_cheer.wav"))
...
def yellow_handle_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_a] and yellow.x - VEL > 0:  # s
        yellow.x -= VEL
    if (
...
def draw_start():
    draw_start = WINNER_FONT.render("START!", 1, WHITE)
    WIN.blit(
        draw_start,
        (
            WIDTH / 2 - draw_start.get_width() / 2,
            HEIGHT / 2 - draw_start.get_height() / 2,
        ),
    )
    pygame.display.update()
...
def main():
    ...
    draw_start()
    yellow = pygame.Rect(50, 100, SPACESHIP_WIDTH, SPACESHIP_HEIGHT)
